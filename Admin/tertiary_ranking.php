            ...
            <tbody>
                <?php
                    ...
                    while ($row = mysqli_fetch_assoc($result)):
                        ...
                        // Fetch points, src_score, and crtc_score from tbl_tertiary_score based on criteria_id and emp_id
                        $criteria_id = $row['criteria_id'];
                        $points = 0;  // Default value for points
                        $src_score = NULL;
                        $crtc_score = NULL;

                        // Query to get points, src_score, and crtc_score for the current criteria_id and emp_id from tbl_tertiary_score
                        $points_query = "SELECT points, src_score, crtc_score FROM tbl_tertiary_score WHERE criteria_id = ? AND emp_id = ?";
                        $stmt = mysqli_prepare($con, $points_query);
                        mysqli_stmt_bind_param($stmt, "ii", $criteria_id, $emp_id);  // Bind parameters
                        mysqli_stmt_execute($stmt);
                        $points_result = mysqli_stmt_get_result($stmt);

                        // Check if a row was returned and get the points, src_score, and crtc_score values
                        if ($points_row = mysqli_fetch_assoc($points_result)) {
                            $points = $points_row['points'];
                            $src_score = $points_row['src_score'];
                            $crtc_score = $points_row['crtc_score'];
                        }
                ?>
                    <tr>
                        ...
                        <td>
                            <input type="number" name="src_score[<?php echo $criteria_id; ?>]" value="<?php echo isset($src_score) ? $src_score : ''; ?>" class="form-control">
                        </td>
                        <td>
                            <input type="number" name="crtc_score[<?php echo $criteria_id; ?>]" value="<?php echo isset($crtc_score) ? $crtc_score : ''; ?>" class="form-control">
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
